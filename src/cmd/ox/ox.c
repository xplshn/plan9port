...
static int
cmpent(const void* a, const void* a2)
{
	Dir*	s1 = a;
	Dir*	s2 = a2;
...
static int
lrueditcmp(const void* a1, const void* a2)
{
	Edit**	e1 = a1;
	Edit**	e2 = a2;
...
cleanpolicy(void)
{
	...
	for(i = 0; i < nedits; i++)
		if(e = edits[i])
			if(e->status.sts == Stemp && cdone(e, 0, nil, 0)){
				if(--nused <= Dnwins)
					break;
			} else
				if(e->status.sts != Sdirty && now - e->atime > Dtime)
...
closefile(Edit* e, int exiting)
{
	...
	if(!exiting)
	if(e->status.sts != Stemp && !(e->qid.type&QTDIR))
...
threadmain(int argc, char* argv[])
{
	...
	inittagcmds();
	homedir = getenv("home");
	if(homedir == nil) homedir = getenv("HOME");
	cleanname(homedir);
	plumbinit();
	cmdinit();
