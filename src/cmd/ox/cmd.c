		...
		if (xcmds[i] && xcmds[i]->pid == pid){
			...
			if (!ok || (time(nil) - xcmds[i]->when > 2)) {
				...
				seprint(vmsg, vmsg+sizeof(vmsg),
					"%s %s\n", xcmds[i]->cmd,
					ok ? "completed" : "failed");
				writefstr("/tmp/omero/voice/output", vmsg);
			}
			freexcmd(xcmds[i]);
			xcmds[i] = nil;
			break;
...
xcmd(char* name, char* dir, char* arg, char* in, char* out, char* ox)
{
	...
	procrfork(xcmdproc, x, 256*1024, RFFDG|RFNOTEG|RFENVG);
	x->pid = recvul(x->pidc);
	...
	if (x->outfd[0] >= 0){
		threadcreate(xcmdoutthread, x, 256*1024);
	}
}
...
cput(Edit* e, int argc, char *[], int force)
{
	...
	if (e->qid.type&QTDIR)
		return 1;
	to = gettagpath(e);
	if (!strcmp(to, e->name) && e->status.sts == Stemp)
		return 1;
	d = dirstat(to);
	if (d != nil){
		if (!force)
...
cdone(Edit* e, int , char* [], int force)
{
	...
	if (e->status.sts != Stemp)
	if (!(e->qid.type&QTDIR))
		if (getsts(e->gtext, &e->status) == Sdirty){
			if (!force){
				s = gettagpath(e);
...
cexit(Edit* , int , char* [], int force)
{
	...
	for(i = 0; i < nedits; i++)
		if(edits[i] && edits[i]->status.sts == Sdirty && !force){
...
cget(Edit* e, int argc, char* argv[], int force)
{
	...
	if (e->status.sts == Stemp)
		return 1;
	if (!force && !(e->qid.type&QTDIR) && e->status.sts == Sdirty){
...
cmdinit(void)
{
	...
	threadcreate(waitthread, nil, 256*1024);
	threadcreate(xcmdsthread, nil, 256*1024);
}
