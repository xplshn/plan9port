...
xdraw(Panel* p, int )
{
	if (hidden(p->file) || Dx(p->rect) <= 0 || Dy(p->rect) <= 0)
		return;
	if (p->draw.canvas && p->rect.max.x)
		draw(screen, p->rect, p->draw.canvas, nil, ZP);
	if (p->flags&Ptag)
		drawtag(p, 0);
}
...
{
	return genreadbuf(buf, count, off, p->draw.dcmds, p->draw.dsize);
}
...
xwrite(Panel* p, void* buf, long cnt, vlong off)
{
	...
	if (off + cnt + 1> p->draw.dsize){
		p->draw.dcmds = erealloc9p(p->draw.dcmds, off + cnt + 1);
		p->draw.dsize = off + cnt;
	}
	memmove(p->draw.dcmds + off, buf, cnt);
	p->draw.dcmds[off + cnt] = 0;

	if (p->size.x > 0){
		p->dfile->dir.length = strlen(p->draw.dcmds);
		drawcmds(p->draw.dcmds, nil, &max);
		r = Rpt(ZP, max);
		...
		drawcmds(p->draw.dcmds, nimage, &max);
		p->minsz = max;
		p->draw.dloaded = 1;
		old = Rpt(ZP, ZP);
		if (p->draw.canvas){
		...
		p->draw.canvas = nimage;
		if (!hidden(p->file)){
			if (eqrect(old, p->draw.canvas->r)){
				xdraw(p, 0);
				putscreen();
			} else
				resize();
...
xmouse(Panel*p, Cmouse* m, Channel* mc)
{
	...
	if ((b = m->mouse.buttons) && cookclick(m, mc))
		event(p, "click %11d %11d %11d %11d", pt.x, pt.y, b, m->mouse.msec);
}
