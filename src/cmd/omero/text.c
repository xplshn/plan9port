...
static Rune
textreadc(Panel *p, int q)
{
	int n;
	Tblock *b;

	b = blockseek(p->frame.blks, &n, q);
...
static int
textbacknl(Panel *p, int q0, int n)
{
	int i;

	i = q0;
	while(i > 0 && textreadc(p, i-1) != '\n'){
		i--;
		if(n==1 && iswordchar(textreadc(p, i)))
			break;
	}
	if(n == 1){
		while(i > 0 && !iswordchar(textreadc(p, i-1)) && textreadc(p, i-1) != '\n')
			i--;
		while(i > 0 && iswordchar(textreadc(p, i-1)))
			i--;
	}
	return i;
}
...
xinit(Panel* p)
{
	...
	if (p->flags&Pline){
		...
		if (q)
			*q = 0;
		p->frame.blks = str2block(s ? s+1 : p->name);
		if (q)
			*q = '.';
	} else
		p->frame.blks = str2block("");
	assert(p->frame.blks);
	if (p->flags&Pline)
...
xdraw(Panel* p, int resize)
{
	...
	if (!p->file)
		return;
	if (p->dfile && p->dfile->dir.length == 0LL)
		p->dfile->dir.length = blocksize(p->frame.blks);
	...
	if (p->frame.fimage == nil){
		...
		if (p->frame.fimage == nil)
			sysfatal("xdraw: allocimage failed");
	...
	draw(screen, p->rect, p->frame.fimage, nil, ZP);
	if (p->flags&Ptag)
		drawtag(p, 0);
}
...
drawscrollbar(Panel* p)
{
	...
	l = p->dfile->dir.length ? p->dfile->dir.length : 1;
	y0 = ysz * p->frame.froff / l;
	dy = ysz * p->frame.f.nchars / l;
	if (dy < 3)
		dy = 3;
	bar.min.y = r.min.y + y0;
	bar.max.y = bar.min.y + dy;
	draw(screen, bar, cols[SET], nil, ZP);
...
jumpscale(Panel* p, Point xy)
{
	...
	dc = blocklen(p->frame.blks) - p->frame.froff;
	if (dc < p->frame.froff)
		dc = p->frame.froff;
	if (dy < 1)
		dy = 1;
...
cut(Panel* p, int putsnarf)
{
	...
	if (!framehassel(p))
		return 0;
	free(p->frame.snarf);
	p->frame.nsnarf = p->frame.se - p->frame.ss;
	p->frame.snarf = malloc((p->frame.nsnarf+1) * sizeof(Rune));
	if (p->frame.nsnarf > 0){
		textdel(p, p->frame.snarf, p->frame.nsnarf, p->frame.ss);
		addedit(p, Del, p->frame.snarf, p->frame.nsnarf, p->frame.ss);
	} else
		p->frame.snarf[0] = 0;
	fillframe(p);
	setframesel(p, p->frame.ss, p->frame.ss, 1);

	if (putsnarf){
		p->frame.snarf[p->frame.nsnarf] = 0;
		s = smprint("%S", p->frame.snarf);
...
paste(Panel* p, int pos)
{
	...
	if (s = readfstr("/dev/snarf")){
		...
		free(p->frame.snarf);
		p->frame.snarf = r;
		p->frame.nsnarf = runeslen(r);
		if (p->frame.nsnarf){
			textins(p, p->frame.snarf, p->frame.nsnarf, pos);
			setframesel(p, pos, pos + p->frame.nsnarf, 1);
			addedit(p, Ins, p->frame.snarf, p->frame.nsnarf, pos);
		}
	}
}
...
xmouse(Panel* p, Cmouse* m, Channel* mc)
{
	...
	panelok(p);
	if (p->frame.fimage == nil || hidden(p->file))	// paranoia
		return;
	if (Dx(p->rect) <= 0 || Dy(p->rect) <= 0) // more paranoia
		return;
	xy = subpt(m->mouse.xy, p->rect.min);
	pos = p->frame.froff + frcharofpt(&p->frame.f, xy);

	if (m->mouse.buttons == 1){
		newedit(p);
		if (m->flag == CMdouble){
		...
		for(;;){
			...
			if (m->mouse.buttons == 0)
				break;
			if (m->mouse.buttons == 1 && !didcut && !didpaste){	// slide
				xy = subpt(m->mouse.xy, p->rect.min);
				pos = p->frame.froff + frcharofpt(&p->frame.f, xy);
				addframesel(p, pos);
				if (p->frame.ss != p->frame.se && !updated++)
					writepsel(p);
			}
			if (m->mouse.buttons == 3){	// 1-2 chord
				if (didpaste){
					if (undo(p))
					...
					didcut++;
					mselcut(p, p->frame.ss);
				}
			}
			if (m->mouse.buttons == 5){	// 1-3 chord
				if (didcut){
					if (undo(p))
						dirty(p, hasedits(p), 0);
					mterm(p, m, mc);
		...
		for(;;){
			...
			if (m->mouse.buttons == 3){	// 2-1 chord
				run(p, pos, 1);
				return;
			}
			xdraw(p, 0);
		...
		run(p, updated ? p->frame.ss : pos, 0);
	} else if (m->mouse.buttons == 4){
		newedit(p);
		updated = 0;
		jfactor = 0;
		jy = m->mouse.xy.y;
		for(;;){
			...
			if (m->mouse.buttons == 0)
				break;
			if (m->mouse.buttons == 4){	// slide
				updated++;
				if (jfactor == 0){
					pos = p->frame.froff;
					jfactor = jumpscale(p, m->mouse.xy);
				}
				jpos = pos + (m->mouse.xy.y - jy) * jfactor;
				jump(p, jpos);
				xdraw(p, 0);
				drawscrollbar(p);
			}
			if (m->mouse.buttons == 5){	// 3-1 chord
				updated++;
				xy = subpt(m->mouse.xy, p->rect.min);
				pos = p->frame.froff + frcharofpt(&p->frame.f, xy);
				addframesel(p, pos);
...
xkeyboard(Panel* p, Rune r)
{
	...
	case Kbs:
		...
		if (p->frame.ss == p->frame.se){
			if (pos - p->frame.froff <= 0 && !scrollframe(p, -3)){
				fdprint("top\n");
				return;
			}
			if (pos > 0){
	...
	case 0x17:	// ^W. erase word
		if (!(p->flags&Pedit))
			goto kbevent;
		if (p->frame.ss == p->frame.se){
			q0 = textbacknl(p, p->frame.ss, 1);
			setframesel(p, q0, p->frame.ss, 0);
		}
		cut(p, 1);
		goto fill;
	...
	case Kdel:
		...
		if (p->frame.ss == p->frame.se){
			if (pos - p->frame.froff >= p->frame.f.nchars && !scrollframe(p, 3)){
				fdprint("bottom\n");
				return;
			}
			textdel(p, buf, 1, pos);
	...
	case Kleft:
		...
		return;
	case Kright:
		if (p->frame.se < blocklen(p->frame.blks)){
			pos = p->frame.se + 1;
			setframesel(p, pos, pos, 1);
			jumpframesel(p);
		}
	...
	case Kesc:
		if (!(p->flags&Pedit))
			goto kbevent;
		setframesel(p, p->frame.mpos, pos, 0);
		break;
	case '\n':
		if (p->flags&Pline){
			setframesel(p, p->frame.mpos, pos, 0);
...
xwrite(Panel* p, void* buf, long cnt, vlong off)
{
	...
	if (off == 0LL)
		p->frame.npartial = 0;
	if (p->frame.npartial){
		if (p->frame.partialoff == off){
			pb = emalloc9p(cnt + p->frame.npartial);
			memmove(pb, p->frame.partial, p->frame.npartial);
			memmove(pb + p->frame.npartial, buf, cnt);
		} else {
			p->frame.npartial = 0;
			p->frame.partialoff = 0;
			werrstr("partial rune");
			return -1;
		}
	}
	nout = p->frame.npartial + cnt;
	r = utf2runes(pb, &nout, &nl);
	if (nout != p->frame.npartial + cnt){
		n = p->frame.npartial + cnt - nout;
		...
		p->frame.npartial = n;
		memmove(p->frame.partial, pb + nout, p->frame.npartial);
		p->frame.partialoff = off + cnt;
	} else {
		p->frame.npartial = 0;
		p->frame.partialoff = 0;
	}
	...
	if (off == 0LL){
		pos = 0;
		blockfree(p->frame.blks);
		p->frame.blks = newblock(r, nr);
		p->frame.nlines = nl;
	} else {
		pos = off2pos(p->frame.blks, (long)off);
	...
	p->dfile->dir.length = off+cnt;
	assert(pos + nr == blocklen(p->frame.blks));
	if (p->flags&Pline)
		pos = pos + nr;
	else
		pos = 0;
...
{
	blockfree(p->frame.blks);
	p->frame.blks = newblock(nil, 0);
	p->frame.nlines = 0;
	p->frame.npartial = 0;
	p->frame.mark = 0;
	p->dfile->dir.length= 0LL;
	reloadframe(p, 0);
	setframemark(p, 0);
	setframesel(p, 0, 0, 1);
}
...
textctlwr(Panel* p, char* s)
{
	...
	if (!strncmp(s, "ins ", 4)){
		...
		if (!strncmp(s + 4, "all", 3)){
			ss = 0;
			se = blocklen(p->frame.blks);
		} else {
			ss = strtod(s + 4, &q);
			se = strtod(q, &r);
			ss = off2pos(p->frame.blks, ss);
			se = off2pos(p->frame.blks, se);
		}
		setframesel(p, ss, se, 1);
		jumpframesel(p);
		xdraw(p, 0);
		...
		setframemark(p, pos2off(p->frame.blks, atoi(s + 5)));
		return cnt;
...
gettextht(Panel* p)
{
	if (p->frame.f.maxlines)
		return p->frame.f.maxlines;
	else
...
xattrs(Panel* p, char* str, long l)
{
	...
	seprint(sel, sel+sizeof(sel),
		"mark %11ld\nsel %11ld %11ld\nsize %11d %11d\n",
		pos2off(p->frame.blks, p->frame.mark),
		pos2off(p->frame.blks, p->frame.ss), pos2off(p->frame.blks, p->frame.se),
