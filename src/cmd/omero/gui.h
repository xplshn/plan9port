...
enum {
	...
	CHigh	= 0xEEEE99FF,	// Plan 9 yellow selection
...
struct Panel {
	Ref r;		// one per file holding it in its aux field.
...
void            initui(void (*derror)(Display*, char*));
int		undo(Panel* p);
void		usage(void);
...
#define	ispanel(f)	((f) && ((f)->dir.qid.type&QTDIR) && (f)->aux)
