...
fileclass(File* f)
{
	return ((f->dir.qid.path&0xff000000)>>24);
}

void
setfileclass(File* f, int id)
{
	f->dir.qid.path |= (((id)&0xff)<<24);
}
...
fsopen(Req* r)
{
	...
	if ((type&1) == 0 && (r->ifcall.mode&OTRUNC) && panels[p->type]->truncate){
		wlock(&p->dfile->rwlock);
		panels[p->type]->truncate(p);
		wunlock(&p->dfile->rwlock);
	}
	if ((f->dir.qid.type&QTDIR) || (type&1) || panels[p->type]->writeall == nil){
		respond(r, nil);
		return;
	}
	/*
...
fscreate(Req* r)
{
	...
	name = r->ifcall.name;
	uid = r->fid->uid;
	mode = d->dir.mode & 0x777 & r->ifcall.perm;
...
fsread(Req* r)
{
	...
	panelok(cp);
	if (type&1){
		rlock(&cp->dfile->rwlock);
		n = panels[cp->type]->attrs(cp, buf, sizeof(buf)-1);
		runlock(&cp->dfile->rwlock);
		if (n < 0){
			respond(r, r->error);
			return;
		}
...
fsthread(void*)
{
	...
	for(;;){
		switch(alt(a)){
		case 0:
			threadcreate(fsreqthread, r, 256*1024);
			break;
		case 1:
...
mklayout(File* f)
{
	...
	d = newfspanel(f, "row:stats", f->dir.uid);
	p = d->aux;
	p->flags |= Ptag|Playout;
	if (smallomero)
		dd = newfspanel(d, "col:cmds", f->dir.uid);
	else
		dd = newfspanel(d, "row:cmds", f->dir.uid);
	for (i = 0; i < nelem(menu); i++){
		seprint(name, name+sizeof(name), "button:%s", menu[i]);
		c = newfspanel(dd, name, f->dir.uid);
		closefile(c);
	}
	closefile(dd);
	closefile(d);

	d = newfspanel(f, "row:wins", f->dir.uid);
	p = dd->aux;
	p->flags |= Playout;
	dd = newfspanel(d, "col:1", f->dir.uid);
	p = dd->aux;
	p->flags |= Playout;
	closefile(dd);
	if (!smallomero){
		dd = newfspanel(d, "col:2", f->dir.uid);
		p = dd->aux;
		p->flags |= Playout;
		closefile(dd);
	}
...
runoxloop(char* p)
{
	...
	procrfork(startproc, "ox", 256*1024, RFPROC|RFNAMEG|RFENVG);
	oxpids[0] = recvul(pidc);
	if (p != nil){
		proccreate(startproc, p, 256*1024);
		recvul(pidc);
	}
	while(prog=recvp(startc)){
		...
		if (remote){
			*remote++ = 0;
			...
			procrfork(startproc, buf, 256*1024, RFPROC|RFNAMEG|RFENVG);
...
olistenproc(void *v)
{
	...
	ctl = announce(((Osrv*)os)->addr, dir);
	if(ctl < 0){
		ctl = announce("tcp!*!0", dir);
		if (ctl < 0)
			sysfatal("announce %s: %r", ((Osrv*)os)->addr);
	}
	...
	if(ni != nil){
		...
		char *p = strrchr(((Osrv*)os)->addr, '!');
		if(p != nil && atoi(p+1) > 0){
			addr = smprint("tcp!%s!%s", sysname(), p+1);
		} else {
			addr = smprint("tcp!%s!11007", sysname());
		}
		saddr = addr;
	}
	rendezvous((ulong)olistenproc, 0);
	for(;;){
		nctl = listen(dir, ndir);
		if(nctl < 0){
			fprint(2, "%s: listen %s: %r", argv0, ((Osrv*)os)->addr);
			break;
		}
		...
		s = (Srv*)emalloc9p(sizeof(Osrv));
		*(Osrv*)s = *(Osrv*)os;
		((Osrv*)s)->addr = getremotesys(ndir);
		s->infd = s->outfd = data;
		s->fpool = nil;
		s->rpool = nil;
		s->rbuf = nil;
		s->wbuf = nil;
		proccreate(osrvproc, s, 256*1024);
	}
	free(((Osrv*)os)->addr);
	free(os);
}
...
othreadlistensrv(Srv *os, char *addr)
{
	...
	s = (Srv*)emalloc9p(sizeof(Osrv));
	*(Osrv*)s = *(Osrv*)os;
	((Osrv*)s)->addr = estrdup9p(addr);
	proccreate(olistenproc, s, 256*1024);
	rendezvous((ulong)olistenproc, 0);
}
...
{
	USED(d);
	fprint(2, "omero: draw error: %s\n", errorstr);
	threadexitsall(errorstr);
}
...
threadmain(int argc, char **argv)
{
	...
	if (srv == nil && mnt == nil){
		char *ns = getns();
		mnt = smprint("%s/omero", ns);
		int fd = create(mnt, OREAD, 0777|DMDIR);
		if(fd >= 0) close(fd);
		char *ports = smprint("%s/omero/ports", ns);
		fd = create(ports, OREAD, 0777|DMDIR);
		if(fd >= 0) close(fd);
		free(ports);
	}
	if (vname != nil)
	...
	if (addr != nil){
		...
		if (vname == nil && !strcmp(addr, "tcp!*!11007"))
			vname = strdup("/tmp/omero/ui");
	}
	if (srv != nil || mnt != nil)
		threadpostmountsrv(&sfs, srv, mnt, mflag);
	if (addr != nil && vname != nil)
		proccreate(announceproc, 0, 256*1024);
