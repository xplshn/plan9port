...
tagmousecmd(File *f, Cmouse* m, Channel* mc)
{
	...
	if (m->mouse.buttons == 1){
		...
		if (!m->mouse.buttons){
			...
			if (m->mouse.buttons == 3 && intag(p, m->mouse.xy)){
				do {
					recv(mc, m);
		...
		slide:
			...
			if (cookslide(m, mc)){
				argcursor(0);
				nf = pointinpanel(m->mouse.xy, innerok);
				if (!innerok)
					nf = paneltop(nf);
				...
				if (nf != f){
					...
					p->flags |= Predraw;
					pfp->flags|= Predraw;
					movefile(f, nf, m->mouse.xy);
				}
			} else
				argcursor(0);
		}
	} else if (m->mouse.buttons == 2 && intag(p, m->mouse.xy)){
		recv(mc, m);
		if (!m->mouse.buttons){
			maxwin(f);
			resize();
	...
	} else  if (m->mouse.buttons == 4 && intag(p, m->mouse.xy)){
		if (cookclick(m, mc)){
			if (!(p->flags&Ptop)){
				p->flags |= Phide;
				pfp = f->parent->aux;
