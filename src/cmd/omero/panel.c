...
hidden(File* f)
{
	...
	if (!f || !(f->dir.qid.type&QTDIR) || !f->aux)
		return 1;
	p = f->aux;
	if (p->flags&Phide)
		return 1;
...
cleanpolicy(File* f)
{
	...
	for (l = w; fp = *l; l++){
		...
		edprint("\t %s\t%ld at %x flags %d wx %d wy\n",
			fp->dir.name, np->atime, np->flags, np->wants.x, np->wants.y);
		if (np->flags&Phide)
			continue;
	...
	edprint("=> f0 %s f1 %s\n", f0 ? f0->dir.name : "nil", f1 ? f1->dir.name : "nil");
	now = time(nil);
	w = newfilewalk(f);
	for (l = w; fp = *l; l++){
		if (!ispanel(fp))
		...
		if (np->type != Qcol /* && np->type != Qrow */)
			continue;
		if (fp != f0 && fp != f1 && np->colrow.nwins != 1){
			edprint("\tminwin %s\n", fp->dir.name);
			minwin(fp);
		} else
			edprint("\t nomin %s (%d nw)\n", fp->dir.name, np->colrow.nwins);
	}
	endfilewalk(w);
}
